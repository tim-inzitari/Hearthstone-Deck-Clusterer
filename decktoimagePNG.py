# Created by Tim Inzitari (@InziHS)
...
def gen_images_parallel(name, deck_dict, dest, ordered):
    ...
    if ordered:
        if (ord(name[0].upper())>=ord('A') and ord(name[0].upper())<=ord('Z')):
            img.save(u'{}/{}/{}.png'.format(dest,name[0].upper(),name), 'PNG')
        else:
            img.save(u'{}/{}/{}.png'.format(dest,'etc',name), 'PNG')
    else:
        img.save(u'{}/{}.png'.format(dest,name), 'PNG')	
